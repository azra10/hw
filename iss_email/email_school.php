...
iss_show_heading_with_backurl("Email School", $backurl);

$current_user = wp_get_current_user();
$accounts = array();

$accounts = ISS_StudentService::GetSchoolEmails();
...
if (isset($_POST['_wpnonce-iss-email-school-form-page'])) {
    iss_write_log(($_POST));
    check_admin_referer('iss-email-school-form-page', '_wpnonce-iss-email-school-form-page');
    ...
    if (!$errSubject && !$errMessage && !$errTo) {
        ...
        echo '<tr><th>Grade</th><th>Student</th><th>Student Email</th><th>School Email</th><th>Second Email</th></tr>';

        $uniqueemails = array();
        foreach ($accounts as $account) {
            echo "<tr><td>Grade{$account->ISSGrade}</td><td>{$account->StudentFirstName} {$account->StudentLastName}";
            echo "</td><td>";
...
<?php wp_nonce_field('iss-email-school-form-page', '_wpnonce-iss-email-school-form-page') ?>
    ...
    <div class="form-group">
		<div class="col-sm-10">
        <label for="to" class="control-label">BCC: <?php echo "School"; ?></label>
      	
       <table class="table-striped table-responsive table-condensed" border=1>      
            <?php 
            echo "<tr><th/><th><span class='text-danger'>{$errTo} </span></th>";
            ...
            foreach ($accounts as $account) {
                ...
                echo "<td>Grade{$account->ISSGrade}</td><td>{$account->StudentFirstName} {$account->StudentLastName}";
                echo "</td><td>";
